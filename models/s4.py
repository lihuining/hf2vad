...
def nplr(measure, N, rank=1, dtype=torch.float):
    ...
    i.e. A = V[w - p q^*]V^*, B = V B,Theorem 1
    """
    A, B = transition(measure, N)
    A = torch.as_tensor(A, dtype=dtype) # (N, N)
    B = torch.as_tensor(B, dtype=dtype)[:, 0] # (N,)

    p = rank_correction(measure, N, rank=rank, dtype=dtype) # (1,N)
...
class SSKernelNPLR(OptimModule):
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        k_f = k_f * 2 / (1 + nodes)
        #print(k_f.shape) # [64, 1, 1, 11] [32, 1, 1, 6] [16, 1, 1, 3]
        k = torch.fft.irfft(k_f)  # (..., 1, 1+s, L) # ?(64,1,1,4)
...
class HippoSSKernel(nn.Module):
    ...
    def __init__(
        ...
        if trainable is not None: # False
        ...
        if mode == "real":  # Testing purposes only
            ...
            C = torch.randn(self.H, self.N // 2, dtype=cdtype) # (64,32)
            if mode == "nplr":
...
class S4(nn.Module):
    ...
    def __init__(
        ...
        self.h = H # 64
        self.n = d_state if d_state > 0 else H # 64
        ...
        self.transposed = transposed # True
        self.resample = resample

        self.D = nn.Parameter(torch.randn(self.h))

        # Optional (position-wise) input transform
        if input_linear: # False
    ...
    def forward(self, u, state=None, cache=None, **kwargs): # absorbs return_output and transformer src mask
        ...
        # Convolution
        k_f = torch.fft.rfft(k, n=2*L) # (H L) n表示填充长度
        u_f = torch.fft.rfft(u, n=2*L) # (B H L)
        y_f = k_f * u_f # 频域相乘,(256,64,21)
